...
public class Bullet {
	...
	private void resultSimulation(int rigidBodyID, int shapeType, float[] rot, float[] pos, float[] shapeOption) {
		...
		if(body == null) {
			System.out.println("resultSimulation "+ "body is null.");
			return;
		}
		if(rot.length < 9) {
			System.out.println("resultSimulation " + "rot is " + rot.length);
			return;
		}
		if(pos.length < 3) {
			System.out.println("resultSimulation "+ "pos is " + pos.length);
			return;
		}
